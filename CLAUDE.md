...
**169 tools, 12 prompts, 21 resources** across 37 domains, allowing Claude
...
└── tools/
    ...
    ├── crud-factory.ts   # Generic CRUD tool factory (registerSearchTool, registerGetTool, registerGetToolMerged, etc.)
...
**Merged GET factory** (`registerGetToolMerged`, `src/tools/crud-factory.ts`):
variant of `registerGetTool` used for entities whose payload is split
across the canonical resource and an `/information` sub-resource
(candidates, contacts, opportunities, companies). When the caller omits
`tab`, the handler fetches the base entity and `/information` in parallel
via `fetchEntityWithInformation()` and returns the merged payload (base
attributes win on conflict, `included[]` unioned by `${type}:${id}`). When
the caller passes `tab` explicitly, the legacy single-tab call is
preserved verbatim — no extra HTTP. Wired in candidates/contacts/
opportunities/companies; all other entities still use `registerGetTool`.

**Information-merge helper** (`fetchEntityWithInformation`, `src/services/boond-client.ts`):
`Promise.allSettled([base, /information])` with non-fatal failure on
`/information` (404, network blip → base response remains intact).
Reusable beyond the merged GET factory — also used internally by tools
that need to enrich a fetched entity before rendering.
...
the runbook — the LLM still drives the calls. Current set (12) :
`synthese_equipe`, `pipeline_commercial`, `factures_a_relancer`,
`factures_en_retard`, `candidats_pour_opportunite`, `fiche_consultant`,
`recap_hebdo`, `staffing_disponible`, `fin_de_mission`,
`cartographie_competences`, `cvs_a_mettre_a_jour`,
`recherche_profil_competences`.

**State map cache** (`getStateMap`, `src/services/dictionary.ts`):
typed wrapper on the dictionary cache that returns `{ byId, byLabel }`
for `setting.state.<entity>` where `<entity>` is one of `candidate`,
`resource`, `contact`, `company`, `opportunity`, `project`, `invoice`,
`order`, `positioning`. Labels are normalized (lowercase + trimmed) for
forgiving lookups. Disabled states (`isEnabled: false`) are skipped.
Powers the textual `stateLabel` shortcut on `boond_candidates_search`
and the unpaid-state filtering in `boond_invoices_overdue`.

**Action type label resolution** (`resolveActionLabel`,
`src/tools/actions.ts`): 3-tier lookup for the action `typeOf` →
human label.
  1. Live dictionary cache (`setting.action.*` merged across linkable
     entity buckets — most accurate, reflects per-org customizations).
  2. Scoped static fallback — `STATIC_TYPE_LABELS_CONTACT` when
     `dependsOn.type === "contact"`, else `STATIC_TYPE_LABELS_CANDIDATE`.
     These mirror the dictionary's per-entity scoping so the same
     numeric ID can mean different things in different contexts.
  3. `type#<id>` placeholder so the UI never shows raw numbers.
Static maps live at the top of `actions.ts` (ported from the legacy
local server boond-mcp-server/index.js).

**Action keyword shortcuts** (`KEYWORD_TO_TYPES`, `src/tools/actions.ts`):
maps a human keyword to one or more `actionTypes[]` IDs (30 categories:
`entretien`, `note`, `appel`, `email`, `relance`, `prospection`, `rdv`,
…). Powers `actionType` on `boond_actions_search` so callers can pass
`actionType: "entretien"` instead of looking up `[19, 12, 22, 23, 133]`.
Explicit `typeOf` (numeric array) always wins ; unknown keywords are
silently ignored.
...
## Ergonomic Shortcuts (server-resolved aliases)

Some endpoints expose textual or behavioral shortcuts that the server
translates to the canonical API parameter before the call. These are
opt-in additions — passing the underlying parameter directly always
takes precedence.

**`stateLabel` on `boond_candidates_search`** — pass a label
(`"Vivier chaud"`, `"Sourcé"`, `"Embauché"`…) instead of looking up
the integer ID. Resolved via `getStateMap("candidate")`. Lookup is
case/whitespace insensitive. Unknown labels are silently ignored.
Explicit `candidateStates: [<id>]` wins.

**`fetchAll: true` (+ `maxResults`) on `boond_candidates_search`** —
opt-in auto-pagination. Forces `pageSize: 500`, walks pages until
`maxResults` (default 500, hard cap 1000) or until a partial page
signals the end of the dataset. Breaks the merged rows into a single
`formatListResponse` output so callers see the same shape as a manual
page. Disabled by default — opt-in only.

**`actionType` on `boond_actions_search`** — textual keyword (`note`,
`appel`, `email`, `entretien`, `relance`, `prospection`, `rdv`,
`signature`…) resolved via `KEYWORD_TO_TYPES` to one or more
`actionTypes[]` IDs. Also accepts a stringified numeric ID
(`actionType: "42"`). Explicit `typeOf` (numeric array) always wins.

**`periodDynamic` on `boond_actions_search`** — enum
(`today`/`yesterday`/`thisWeek`/`lastWeek`/`thisMonth`/`lastMonth`/
`thisQuarter`/`lastQuarter`/`thisYear`/`lastYear`) forwarded as-is to
the API. Combinable with `period` (which chooses the date field:
`started`/`created`/`updated`).

**Linked-entity prefixes on `boond_actions_search`** — `candidateId`,
`resourceId`, `contactId`, `companyId` are injected into `keywords` as
`CAND<id>` / `COMP<id>` / `CCON<id>` / `CSOC<id>` prefixes. BoondManager
ignores the literal `candidateId=` style on `/actions`, so we route
through the keyword prefix syntax (documented in the "Linked entity"
gotcha above). A user-supplied `keywords` value is preserved and
appended after the prefixes.

**Merged `_get` on candidates/contacts/opportunities/companies** —
when `tab` is omitted, the response merges the canonical resource with
its `/information` sub-resource (parallel fetch, base attributes win).
Passing `tab="information"` (or any other tab) keeps the legacy
single-tab behavior. See `registerGetToolMerged` for the implementation.
...
- **Current stats**: 49 test files, **504 tests**
...
4. **GET tool variant**: use `registerGetToolMerged` instead of `registerGetTool` when the entity has an `/information` sub-resource that should be merged with the canonical payload by default (candidates, contacts, opportunities, companies). For all other entities `registerGetTool` is the right choice — keeps the call cost at 1 HTTP request.
5. If the domain is one of the six "search" domains, model the schema on the official RAML
...
7. Register in `src/server.ts`
8. Add API path to `API_PATHS` in `constants.ts` (if applicable)
9. Create `src/tools/{domain}.test.ts` following the test pattern above
10. Regenerate the catalogue: `npm run docs:tools` (commit `TOOLS.md` along with the change — CI fails otherwise)
11. Run `npm test && npm run lint && npm run typecheck && npm run build`
...
## VSM Fork — Deployment Notes (Azure Container Apps)

This fork (`benoitkochversusmind/boondmanager-mcp-vsm`) ships to an Azure
Container App on top of the upstream stdio MCP, adding OAuth Entra ID
auth + per-user JWT injection. VSM-only ops considerations :

- **`minReplicas: 1` is required** (`az containerapp update --min-replicas 1`).
  Setting `minReplicas: 0` enabled a KEDA race: when a new revision was
  deployed, KEDA scaled the pod to zero before the GHCR image pull
  finished (observed 10+ min pull times on France Central → GHCR West),
  leaving the revision in `ActivationFailed` and the public endpoint
  unresponsive. Keeping at least one replica permanently warm prevents
...
- **Single-revision mode** — traffic split via `ingress traffic set`
  ...
  revision visible in `az containerapp revision list` for quick
  rollback.
- **Diagnostic logs** — console output goes to a Log Analytics workspace
  pointed at by the env `properties.appLogsConfiguration`. Use
  `az monitor log-analytics query --workspace <id> --analytics-query
  "ContainerAppConsoleLogs_CL | where ContainerAppName_s == 'ca-boondmcp-vsm'
  | order by TimeGenerated desc | take 50"`. System events
  (KEDA scaling, image pulls) live in `ContainerAppSystemLogs_CL`.
- **Container App coordinates** : RG `rg-corp-ai-shared-prod-001`,
  subscription `cb53f633-22c1-4ec9-b20b-b82bb2d9346a`, region France
  Central, app name `ca-boondmcp-vsm`. Public URL
