...
/**
 * Same as `registerGetTool` but auto-merges the entity with its
 * `/information` tab when no explicit `tab` is requested. Used for entities
 * whose payload is split across the canonical resource and an `information`
 * sub-resource (candidates, contacts, opportunities, companies).
 *
 * When the caller passes `tab` (e.g. `tab="technical-data"`), the legacy
 * single-tab behavior is preserved verbatim — no extra HTTP call. The merge
 * only kicks in on the bare `_get(id)` call, where it makes the response
 * richer at the cost of one parallel GET to `/information`. Failures on the
 * `/information` call degrade gracefully to the base entity.
 */
export function registerGetToolMerged(server: McpServer, opts: CrudToolOptions): void {
  server.registerTool(
    `${opts.prefix}_get`,
    {
      title: `Détails d'un(e) ${opts.entityName}`,
      description: `Récupère les informations détaillées d'un(e) ${opts.entityName} par son ID.

Sans \`tab\`, la sortie agrège automatiquement l'endpoint principal ET l'onglet \`information\` (parallèle, fusion des \`attributes\` et union de \`included[]\`). Si \`tab\` est fourni (ex: 'technical-data', 'positionings'), seul cet onglet est renvoyé.

Args:
  - id (string): Identifiant unique du/de la ${opts.entityName}
  - tab (string, optional): Onglet spécifique à récupérer (sinon merge avec /information)

Returns: Données JSON complètes de l'entité (richesse maximale par défaut).`,
